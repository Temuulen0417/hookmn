...
// Daily free quota tracker — resets at midnight
const useQuota = (isPremium) => {
  const todayKey = () => new Date().toISOString().slice(0, 10);
  const [quota, setQuota] = useLocalStorage("hookmn_quota", { date: todayKey(), used: 0 });
  const FREE_DAILY = 5;

  const getUsed = () => {
    if (isPremium) return 0;
    if (quota.date !== todayKey()) return 0;
    return quota.used || 0;
  };
  ...
  const increment = () => {
    ...
    const base = quota.date === today ? quota.used : 0;
    setQuota({ date: today, used: base + 1 });
  };

  const used = getUsed();
  const remaining = Math.max(0, FREE_DAILY - used);
  const exhausted = !isPremium && remaining === 0;

  return { used, remaining, total: FREE_DAILY, exhausted, increment };
};
    ...
    >
      ...
      {!locked && (
        <div style={{ display: "flex", flexDirection: "column", gap: 6, flexShrink: 0, alignItems: "flex-end" }}>
          ...
          <span style={{ fontSize: 10, color: "#334155", fontVariantNumeric: "tabular-nums" }}>{charCount}c</span>
        </div>
      )}
    </div>
  );
  ...
  <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
    {[0.9, 1, 0.75, 0.85, 0.6].map((w, i) => (
      <div key={i} style={{
        ...
        <div style={{ width: 28, height: 28, borderRadius: 8, background: "rgba(168,85,247,0.15)", flexShrink: 0 }} />
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 6 }}>
          <div style={{ height: 12, borderRadius: 6, background: "rgba(255,255,255,0.06)", width: `${w * 100}%` }} />
          <div style={{ height: 12, borderRadius: 6, background: "rgba(255,255,255,0.04)", width: `${w * 60}%` }} />
        </div>
      </div>
    ))}
  </div>
...
const QuotaBar = ({ used, total, remaining, onUpgrade }) => {
  const pct = Math.min(100, (used / total) * 100);
  const color = pct >= 100 ? "#f43f5e" : pct >= 80 ? "#f59e0b" : "#a855f7";
  return (
    <div style={{
      background: "rgba(255,255,255,0.03)",
      border: `1px solid ${pct >= 100 ? "rgba(244,63,94,0.25)" : "rgba(255,255,255,0.07)"}`,
      ...
      <div style={{ flex: 1, minWidth: 160 }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 800, color: pct >= 100 ? "#f87171" : "#94a3b8" }}>
            {used}/{total}
          </span>
        </div>
      ...
      {pct >= 100 ? (
        <button onClick={onUpgrade} style={{
          ...
          borderRadius: 8, color: "#fff", padding: "6px 14px", fontSize: 11,
...
const ToolsPage = ({ favorites, setFavorites, history, setHistory, isPremium, setShowPremium }) => {
  ...
  // Persist niche + platform across sessions
  const [niche, setNiche] = useLocalStorage("hookmn_niche", "");
  const [platform, setPlatform] = useLocalStorage("hookmn_platform", "TikTok");
  ...
  const FREE_LIMIT = 5;
  const quota = useQuota(isPremium);

  // Keyboard shortcut: Cmd/Ctrl + Enter to generate
  useEffect(() => {
    const handler = (e) => {
      if ((e.metaKey || e.ctrlKey) && e.key === "Enter" && tab === "generate") {
        e.preventDefault();
        generate();
      }
    };
  ...
  }, [tab, topic, loading, quota.exhausted]);

  const showToast = (msg, type = "success") => {
    setToast({ visible: true, msg, type });
    setTimeout(() => setToast({ visible: false, msg: "", type: "success" }), 2500);
  };
  ...
  const exportFavorites = () => {
    ...
    const txt = favorites.map((f, i) =>
      `[${i + 1}] ${TOOLS.find((t) => t.id === f.tool)?.label || f.tool}\n${f.text}\n`
    ).join("\n");
    const blob = new Blob([txt], { type: "text/plain" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a"); a.href = url; a.download = "hookmn-favorites.txt"; a.click();
    URL.revokeObjectURL(url);
    showToast("Татаж авлаа!", "info");
  };

  const generate = () => {
    if (!topic.trim()) { setError("Сэдэв оруулна уу!"); return; }
    if (quota.exhausted) { setShowPremium(true); return; }
    setError("");
    setLoading(true);
    setResults([]);
  ...
  const filteredHistory = historySearch.trim()
    ? history.filter((h) => h.topic.toLowerCase().includes(historySearch.toLowerCase()) || h.tool.includes(historySearch.toLowerCase()))
    : history;
    ...
    <div style={{ maxWidth: 860, margin: "0 auto", padding: "100px 16px 60px" }}>
      ...
      {tab === "generate" && (
        <>
          ...
          {!isPremium && (
            <QuotaBar used={quota.used} total={quota.total} remaining={quota.remaining} onUpgrade={() => setShowPremium(true)} />
          )}
          {/* Tool Selector */}
          ...
          <div style={{ position: "relative", marginBottom: 28 }}>
            <button onClick={generate} disabled={loading || quota.exhausted} style={{
              width: "100%",
              background: quota.exhausted ? "linear-gradient(135deg,#f59e0b,#d97706)"
                : loading ? "rgba(168,85,247,0.3)"
                : "linear-gradient(135deg,#a855f7,#7c3aed)",
              border: "none", borderRadius: 14, color: "#fff", padding: "17px",
              fontSize: 16, fontWeight: 800, cursor: loading ? "not-allowed" : "pointer",
              boxShadow: quota.exhausted ? "0 8px 32px rgba(245,158,11,0.3)"
                : loading ? "none" : "0 8px 32px rgba(168,85,247,0.35)",
              transition: "all 0.2s", fontFamily: "'Space Grotesk',sans-serif",
              display: "flex", alignItems: "center", justifyContent: "center", gap: 10,
            }}>
              {quota.exhausted ? "👑 Лимит дууссан — Pro болох" : loading ? (
                <><span style={{ animation: "spin 1s linear infinite", display: "inline-block" }}>◌</span> AI үүсгэж байна...</>
              ) : `${tool?.icon} ${tool?.labelMn} үүсгэх`}
            </button>
            {!loading && !quota.exhausted && (
              <span style={{
      ...
      {tab === "favorites" && (
        <div>
          {favorites.length > 0 && (
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 8 }}>
              <span style={{ fontSize: 13, color: "#64748b", fontWeight: 600 }}>{favorites.length} хадгалсан</span>
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={exportFavorites} style={{
                  background: "rgba(6,182,212,0.08)", border: "1px solid rgba(6,182,212,0.2)",
                  borderRadius: 8, color: "#67e8f9", padding: "7px 14px", fontSize: 12, fontWeight: 700, cursor: "pointer",
                }}>↓ Татаж авах</button>
                <button onClick={() => { if (window.confirm("Бүгдийг устгах уу?")) { setFavorites([]); showToast("Бүгд устгагдлаа"); }}} style={{
                  background: "rgba(244,63,94,0.06)", border: "1px solid rgba(244,63,94,0.15)",
                  borderRadius: 8, color: "#f87171", padding: "7px 14px", fontSize: 12, fontWeight: 700, cursor: "pointer",
          ...
          {favorites.length === 0 ? (
            <div style={{ textAlign: "center", padding: "60px 20px", color: "#475569" }}>
              ...
              <button onClick={() => setTab("generate")} style={{
                background: "linear-gradient(135deg,#a855f7,#7c3aed)", border: "none",
                borderRadius: 10, color: "#fff", padding: "10px 24px", fontSize: 13,
              ...
              {favorites.map((f, i) => (
                <GlowCard key={i} hover={false}>
                  <div style={{ fontSize: 11, color: "#475569", marginBottom: 8, fontWeight: 700 }}>
                    ...
                    <span style={{ marginLeft: 8, color: "#334155" }}>{new Date(f.ts).toLocaleDateString("mn-MN")}</span>
                  </div>
                  <p style={{ margin: "0 0 12px", color: "#e2e8f0", fontSize: 14, lineHeight: 1.6, whiteSpace: "pre-wrap" }}>{f.text}</p>
                  <div style={{ display: "flex", gap: 8 }}>
      ...
      {tab === "history" && (
        <div>
            ...
            <div style={{ marginBottom: 16 }}>
              <input
                ...
                style={{
                  ...
                  color: "#e2e8f0", padding: "11px 16px", fontSize: 14,
          ...
          {filteredHistory.length === 0 ? (
            <div style={{ textAlign: "center", padding: "60px 20px", color: "#475569" }}>
              <div style={{ fontSize: 48, marginBottom: 16 }}>🕐</div>
              <p>{history.length === 0 ? "Түүх байхгүй байна" : "Хайлт тохирсонгүй"}</p>
              {history.length === 0 && (
                <button onClick={() => setTab("generate")} style={{
                  background: "linear-gradient(135deg,#a855f7,#7c3aed)", border: "none",
                  borderRadius: 10, color: "#fff", padding: "10px 24px", fontSize: 13,
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {filteredHistory.map((h, i) => (
                <GlowCard key={i} hover={false} onClick={() => { setTab("generate"); setActiveTool(h.tool); setTopic(h.topic); }} style={{ cursor: "pointer" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10, flexWrap: "wrap", gap: 8 }}>
                    ...
                    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                      ...
                      <span style={{ fontSize: 10, color: "#a855f7", fontWeight: 600, background: "rgba(168,85,247,0.1)", borderRadius: 6, padding: "2px 8px" }}>дахин үүсгэх →</span>
                    </div>
                  </div>
...
export default function App() {
  ...
  const [isPremium, setIsPremium] = useLocalStorage("hookmn_premium", false);
  // Dark mode — persisted (default: dark=true to match neon UI)
  const [darkMode, setDarkMode] = useLocalStorage("hookmn_darkmode", true);
  ...
  // Theme tokens derived from darkMode
  const theme = {
    bg:       darkMode ? "#070711" : "#f0f0f8",
    bgCard:   darkMode ? "rgba(255,255,255,0.03)" : "rgba(0,0,0,0.04)",
    bgNav:    darkMode ? "rgba(7,7,17,0.92)"     : "rgba(240,240,248,0.95)",
    border:   darkMode ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.10)",
    text:     darkMode ? "#e2e8f0" : "#0f172a",
    textMid:  darkMode ? "#94a3b8" : "#475569",
    textDim:  darkMode ? "#64748b" : "#94a3b8",
  };

  return (
    <div style={{ minHeight: "100vh", background: theme.bg, color: theme.text, fontFamily: "'DM Sans',sans-serif", position: "relative", overflowX: "hidden", transition: "background 0.3s, color 0.3s" }}>
      {/* Global styles */}
      <style>{`
        ...
        * { box-sizing: border-box; }
        body { margin: 0; background: ${theme.bg}; transition: background 0.3s; }
        ::-webkit-scrollbar { width: 6px; }
        ::-webkit-scrollbar-track { background: ${darkMode ? "#070711" : "#e2e8f0"}; }
        ...
        select option { background: ${darkMode ? "#0f0f1a" : "#fff"}; color: ${darkMode ? "#e2e8f0" : "#0f172a"}; }
      ...
      <aside style={{
        ...
        width: Math.min(380, typeof window !== "undefined" ? window.innerWidth * 0.92 : 380),
        background: darkMode ? "rgba(10,10,24,0.98)" : "#ffffff",
        borderLeft: `1px solid ${darkMode ? "rgba(168,85,247,0.2)" : "rgba(0,0,0,0.1)"}`,
        backdropFilter: "blur(24px)",
        transform: showHistorySidebar ? "translateX(0)" : "translateX(110%)",
        transition: "transform 0.35s cubic-bezier(0.4,0,0.2,1)",
        display: "flex", flexDirection: "column",
        boxShadow: showHistorySidebar ? "-20px 0 60px rgba(0,0,0,0.5)" : "none",
      }}>
        {/* Sidebar header */}
        <div style={{
          ...
          <div>
            <div style={{ fontWeight: 900, fontSize: 17, fontFamily: "'Space Grotesk',sans-serif", color: theme.text }}>
              🕐 Үүсгэсэн түүх
            </div>
            <div style={{ fontSize: 12, color: theme.textDim, marginTop: 2 }}>
              {history.length} бичлэг хадгалагдсан
            </div>
          </div>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            {history.length > 0 && (
              <button
                onClick={() => { if (window.confirm("Бүх түүхийг устгах уу?")) setHistory([]); }}
                style={{
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 8, color: theme.textMid,
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: 16 }}>
          {history.length === 0 ? (
            <div style={{ textAlign: "center", padding: "48px 20px", color: theme.textDim }}>
              ...
              <p style={{ margin: 0, fontSize: 14 }}>Түүх хоосон байна</p>
              <p style={{ margin: "6px 0 0", fontSize: 12 }}>Хук үүсгэснийхээ дараа энд харагдана</p>
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {history.map((h, i) => {
                const toolInfo = TOOLS.find((t) => t.id === h.tool);
                return (
                  <div
                    key={i}
                    onClick={() => { navigate("tools"); setShowHistorySidebar(false); }}
                    style={{
                    ...
                    onMouseLeave={(e) => { e.currentTarget.style.borderColor = theme.border; e.currentTarget.style.background = darkMode ? "rgba(255,255,255,0.03)" : "rgba(0,0,0,0.03)"; }}
                  >
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                      <span style={{ fontSize: 11, fontWeight: 700, color: "#a855f7" }}>
                        {toolInfo?.icon} {toolInfo?.labelMn}
                      </span>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                      {h.results.slice(0, 2).map((r, j) => (
                        <div key={j} style={{
                          fontSize: 12, color: theme.textDim,
                          paddingLeft: 8, borderLeft: "2px solid rgba(168,85,247,0.3)",
                          lineHeight: 1.4,
                          overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
                        }}>{r}</div>
                      ))}
                      {h.results.length > 2 && (
                        <div style={{ fontSize: 11, color: theme.textDim, paddingLeft: 8 }}>
                          +{h.results.length - 2} дэлгэрэнгүй
                        </div>
                      )}
                    </div>
                  </div>
        ...
        {history.length > 0 && (
          <div style={{
            padding: "14px 16px",
            borderTop: `1px solid ${darkMode ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.08)"}`,
          }}>
            <button
              onClick={() => {
                const txt = history.map((h, i) =>
                  `[${i + 1}] ${TOOLS.find((t) => t.id === h.tool)?.label} — "${h.topic}" (${h.platform})\n` +
                  h.results.map((r, j) => `  ${j + 1}. ${r}`).join("\n")
                ).join("\n\n");
                const blob = new Blob([txt], { type: "text/plain" });
                const url = URL.createObjectURL(blob);
                const a = document.createElement("a"); a.href = url; a.download = "hookmn-history.txt"; a.click();
                URL.revokeObjectURL(url);
              }}
              style={{
                width: "100%", background: "rgba(6,182,212,0.08)", border: "1px solid rgba(6,182,212,0.2)",
                borderRadius: 10, color: "#67e8f9", padding: "10px",
                fontSize: 13, fontWeight: 700, cursor: "pointer",
              }}
            >↓ Бүгдийг татаж авах</button>
          </div>
        )}
      </aside>
      ...
      <nav style={{
        ...
        display: "flex", alignItems: "center", justifyContent: "space-between",
        background: theme.bgNav, backdropFilter: "blur(24px)",
        ...
        <button onClick={() => navigate("home")} style={{ background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 8, padding: 0 }}>
          ...
          <span style={{ fontSize: 18, fontWeight: 900, color: theme.text, fontFamily: "'Space Grotesk',sans-serif", letterSpacing: -0.5 }}>
        ...
        <div className="desktop-nav" style={{ display: "flex", gap: 4, alignItems: "center" }}>
          {NAV.map((n) => (
            <button key={n.id} onClick={() => navigate(n.id)} style={{
              ...
              borderRadius: 10, color: page === n.id ? "#c084fc" : theme.textDim,
              padding: "7px 14px", fontSize: 13, fontWeight: 700, cursor: "pointer", transition: "all 0.2s",
            }}
              onMouseEnter={(e) => { if (page !== n.id) { e.currentTarget.style.color = theme.textMid; e.currentTarget.style.borderColor = theme.border; } }}
              onMouseLeave={(e) => { if (page !== n.id) { e.currentTarget.style.color = theme.textDim; e.currentTarget.style.borderColor = "transparent"; } }}
            >{n.label}</button>
          ))}
          ...
          <button
            ...
            style={{
              background: darkMode ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.06)",
              border: `1px solid ${theme.border}`,
              borderRadius: 10, color: theme.textMid,
              padding: "7px 10px", fontSize: 15, cursor: "pointer",
          ...
          >
            ...
            {history.length > 0 && (
              <span style={{
                ...
                borderRadius: 10, padding: "1px 5px", fontSize: 9, fontWeight: 800, color: "#fff",
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <button
            onClick={() => setDarkMode(!darkMode)}
            style={{
              background: "none", border: "none", color: theme.textMid,
          ...
          <button
            ...
            style={{
              ...
              borderRadius: 8, color: "#c084fc", padding: "6px 10px", fontSize: 14, cursor: "pointer",
              position: "relative",
            }}
          >
            🕐
            {history.length > 0 && (
              <span style={{
                position: "absolute", top: -4, right: -4,
                background: "#a855f7", borderRadius: 8, padding: "1px 4px",
          ...
          <button className="hamburger" onClick={() => setMenuOpen(!menuOpen)} style={{
            display: "none", background: darkMode ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.06)",
            border: `1px solid ${theme.border}`,
            borderRadius: 10, color: theme.text, padding: "8px 14px", cursor: "pointer", fontSize: 18,
          }}>{menuOpen ? "✕" : "☰"}</button>
        </div>
      </nav>
      ...
      {menuOpen && (
        <div style={{
          ...
          background: darkMode ? "rgba(7,7,17,0.98)" : "rgba(240,240,248,0.98)",
          backdropFilter: "blur(24px)",
          borderBottom: `1px solid ${theme.border}`,
          ...
          {NAV.map((n) => (
            <button key={n.id} onClick={() => navigate(n.id)} style={{
              ...
              border: "none", color: page === n.id ? "#c084fc" : theme.textMid,
